...
// ======================================================
// 📢 Announcements.jsx — Admin real-time announcements
// ======================================================
import React, { useState, useEffect, useContext } from "react";
import axios from "axios";
import { AuthContext } from "../../AuthContext";
import { io } from "socket.io-client";
import "./Announcements.css"; // ✅ external styling
...
const Announcements = () => {
  ...
  // ✅ On mount: fetch and connect to Socket.IO
  useEffect(() => {
    fetchAnnouncements();
    ...
    // When new announcement is added anywhere (auto-update)
    s.on("announcement:new", (data) => {
      console.log("📨 New announcement:", data);
      setAnnouncements((prev) => [data, ...prev]);
    });

    // When announcement is deleted elsewhere
    s.on("announcement:deleted", (id) => {
  ...
  const handleAddAnnouncement = async () => {
    if (!newAnnouncement.trim()) return alert("Please enter announcement text.");
  ...
  const deleteAnnouncement = async (id) => {
    ...
    try {
      ...
      // no need to refetch — socket handles it
    } catch (err) {
      console.error("Error deleting announcement:", err);
      alert("Failed to delete");
    }
  ...
  if (loading)
    return (
      <h2 className="loading-text">Loading announcements...</h2>
    );
